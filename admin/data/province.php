...
<?php
  ...
  if(isset($_POST['Save'])){
    $obj->insert_province(trim($_POST['P_M_Name']));
  }
  if(isset($_POST['btnUpdate'])){
    $obj->update_province(trim($_POST['P_M_ID_update']),trim($_POST['P_M_Name_update']));
  }
        ...
        $logic = 0;
            ...
            if(isset($_POST["btSelectItem"])){
                foreach($_POST["btSelectItem"] as $checkid){
                    $check_userDemand = mysqli_query($conn,"select * from demand_user where p_m_id='$checkid'");//ກວດສອບອີເມວວ່າມີແລ້ວ ຫຼື ຍັງ
                    $check_quota = mysqli_query($conn,"select * from quota_distribute where p_m_id='$checkid'");//ກວດສອບອີເມວວ່າມີແລ້ວ ຫຼື ຍັງ
