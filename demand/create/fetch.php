...
$path = "../../";
 ...
 if($_POST["query"]){
    ...
    $row_fy2 = mysqli_fetch_array($fy2,MYSQLI_ASSOC);
     $id = $row_fy2["dept_id"];
     $no_ = 0;
     $unitversity = mysqli_query($conn,"select * from university where dept_id='$id' ORDER BY uni_name ASC;");
...
<div class="table-responsive tscroll">
    <table id="detail_table" class="table-bordered" style="width: 1460px;">
        ...
        <?php
                $unit_id = $uni['uni_id'];
                $i = 0;
                $faculty = mysqli_query($conn,"select * from faculty where uni_id='$unit_id' ORDER BY fac_name ASC;");
        ...
        <?php
                $fac_id = $fac['fac_id'];
                $j = 0;
                $course = mysqli_query($conn,"select * from course c left join course_level lv on c.level_id=lv.level_id where fac_id='$fac_id' ORDER BY level_name ASC,course_name ASC;");
        ...
        <tr class="child-<?php echo $fac['fac_id'] ?>" style="display: table-row;">
            ...
            <td style="text-align:center">
                <input class="quantity" min="0" name="num1" id="total_course_<?php echo $cour['course_id'] ?>" value="0"
                    type="number" style="border:none;width:100%;max-width:180px;text-align:center;font-weight: bold;background:none;"
                    disabled>
            </td>
            <td>
...
$('.demandBtn').on('click', function() {
    ...
    $u = mysqli_query($conn,"select * from university where dept_id='$id';");
    foreach($u as $un){
        $u_id = $un['uni_id'];
        $f = mysqli_query($conn,"select * from faculty where uni_id='$u_id'");
        ...
        foreach($f as $fc){
            ...
            foreach($c as $cr){
                ...
                //ດຶງຂໍ້ມູນສາຂາວິຊາທີ່ຢູ່ມີເງື່ອນໄຂຢູ່ໃນຂະນະທີ່ ຄິກອອກເພື່ອເອົາມາບວກກັນ
                $f_sql = mysqli_query($conn,"select * from faculty where uni_id='$u_id'");
    ...
    $unit_total = mysqli_query($conn,"select * from university where dept_id='$id';"); //ດຶງຂໍ້ມູນມະຫາໄລທັງໝົດອອກມາ
    ?>
